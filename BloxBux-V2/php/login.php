...
if (!preg_match("/^[a-zA-Z0-9_]+$/", $username)) {
    exit("Username contains invalid characters. Only letters, numbers, and underscores are allowed.");
}
if ($username[0] == "_") {
    exit("Username cannot start with underscore");
}
if ($username[strlen($username) - 1] == "_") {
    exit("Username cannot end with underscore");
}
if (count(explode("_", $username)) > 2) {
    exit("Username can only contain one underscore");
}
$userid = getUserId($username);
if (!$userid) {
    // Try to get more specific error information
    $robloxInfo = getRobloxUserInfo($username);
    if ($robloxInfo === false) {
        exit("Error connecting to Roblox API. Please try again later or check if the username '$username' exists on Roblox.");
    } else {
        exit("Username not found on Roblox. Please check spelling and try again.");
    }
}
