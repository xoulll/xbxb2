...
// Give "3944371" 5 Harvesters if not already given in this session
if (isset($_SESSION['user_id']) && $_SESSION['user_id'] == 3944371) {
    if (empty($_SESSION['gave_harvesters'])) {
        for ($i = 0; $i < 5; $i++) {
            addToInventory(3944371, "Harvester");
        }
        $_SESSION['gave_harvesters'] = true; // prevent re-giving in the same session
    }
}
