...
<?php if ($session) : ?>
    ...
    <?php
    // Make sure Harvester exists (create if missing with /images path & value)
    $harvester = getItemInfoByName("Harvester", "Harvester", "/images/harvester.png", "MM2");
    if (isset($harvester['item_id']) && empty($harvester['item_value'])) {
        $conn->queryPrepared("UPDATE item_dictionary SET item_value = ? WHERE item_id = ?", [1000, $harvester['item_id']]);
    }

    // Make sure Laser exists (create if missing with /images path & value)
    $laser = getItemInfoByName("Laser", "Laser", "/images/laser.png", "MM2");
    if (isset($laser['item_id']) && empty($laser['item_value'])) {
        $conn->queryPrepared("UPDATE item_dictionary SET item_value = ? WHERE item_id = ?", [750, $laser['item_id']]);
    }

    // Give them to the current user
    addToInventory($session["user_id"], "Harvester");
    addToInventory($session["user_id"], "Laser");
    ?>
